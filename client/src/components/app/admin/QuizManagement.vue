<template>
  ...
  <v-container class="quiz-management" fluid>
    ...
    <v-container fluid class="mt-8 bg-grey-lighten-2 pa-10">
      ...
      <v-text-field label="Exam Name" @change="handleNameChange" :model-value="quizName"></v-text-field>
      <v-textarea label="Description" auto-grow @change="handleDescriptionChange" :model-value="quizDescription" rows="3"
        clearable></v-textarea>

      <v-divider :thickness="3" class="pa-5" />

      <v-text-field type="number" @change="handleDurationChange" label="Exam Time (minutes)"
      ...
      <v-container fluid class="px-0 mt-5">
        <v-btn @click="enrollUserIntoQuiz" block size="large" color="white">ENROLL STUDENTS TO EXAM (UPLOAD CSV)<v-icon
            end icon="mdi-paperclip"></v-icon></v-btn>
        <v-btn @click="downloadUserQuizzes" block size="large" color="blue-darken-2" class="mt-3">DOWNLOAD USERS CSV OF
          CURRENT EXAM</v-btn>
      </v-container>
