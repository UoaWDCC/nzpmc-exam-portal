<template>
    <div class="pre-exam">
        ...
        <div v-if="!examCompleted" class="start-section"> <!-- Renders if exam is not completed -->
            <h3 id="exam-duration">Exam Duration: {{ examDuration }}</h3>
            <v-btn :to="{ name: 'AppExam', params: { quizID: examID } }" id="start-btn" variant="tonal">START EXAM</v-btn>
            <p id="exam-warning">Clicking the start button will begin the quiz timer. Good luck!</p>
        </div>

        <div v-if="examCompleted" class="start-section"> <!-- Renders if exam is completed -->
            <h3 id="exam-duration">Time Used: {{ examTimeUsed }}</h3>

            <span v-if="examMarked" class="start-section"> <!-- Renders if examMarked is true -->
                ...
                <v-btn :to="{ name: 'AppExam', params: { quizID: examID } }" id="start-btn" variant="tonal">VIEW DETAILS</v-btn>
            </span>
            <span v-else class="start-section"> <!-- Renders if examMarked is false -->
...
export default {
    ...
    data(): any {
        return {
            ...
            examDescription: 'this is a description of the quiz, it contains information about what the quiz will be like. Some other random information is contained here. I don’t really know what to write. You are allowed a calculator, you cannot access the internet or use a smart device.',
